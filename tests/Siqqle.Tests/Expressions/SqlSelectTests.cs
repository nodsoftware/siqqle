...
public class SqlSelectTests
{
    ...
    [Fact]
    public void From_WithNullSubqueryAlias_ThrowsArgument()
    {
        var select = Sql.Select("Id", "Name");
    ...
    {
        var select = Sql.Select("Id", "Name").From("Users").Go();
    ...
    {
        var subquery = Sql.Select("Id", "Name").From("Users").Go() + "u";

        var select = Sql.Select("Id", "Name").From(subquery).Go();
    ...
    {
        var select = Sql.Select("Id", "Name").From("User").Where(SqlExpression.Equal("Id", 5)).Go();
        ...
        Assert.Equal(
            "Id",
            (
                (SqlColumn)((SqlBinaryExpression)select.Where.Predicate).Left
            ).ColumnName.Segments.First()
        );

        Assert.Equal(
            SqlBinaryOperator.Equal,
            ((SqlBinaryExpression)select.Where.Predicate).Operator
        );
    ...
    public void Join_WithTableAndPredicate_AddsJoin()
    {
        ...
        SqlSelect select = Sql.Select("Id", "Name").From(users).Join(profiles).On(predicate).Go();

        Assert.Single(select.From.Joins);
    ...
    public void InnerJoin_WithTableAndPredicate_AddsJoin()
    {
        ...
        var profiles = new SqlTable("dbo.Profiles", "p");
        var predicate = SqlExpression.Equal(users + "Id", profiles + "UserId");
        var select = Sql.Select("Id", "Name").From(users).InnerJoin(profiles).On(predicate).Go();

        var join = Assert.Single(select.From.Joins);
    ...
    public void LeftJoin_WithTableAndPredicate_AddsJoin()
    {
        ...
        var profiles = new SqlTable("dbo.Profiles", "p");
        var predicate = SqlExpression.Equal(users + "Id", profiles + "UserId");
        var select = Sql.Select("Id", "Name").From(users).LeftJoin(profiles).On(predicate).Go();
    ...
    {
        var select = Sql.Select("Id", "Name").From("User").OrderBy("Name").Limit(42).Go();
    ...
    {
        var select = Sql.Select("Id", "Name").From("User").OrderBy("Name").Limit(42, 25).Go();
    ...
    public void GroupBy_WithNullColumn_ThrowsArgumentNull()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            Sql.Select("FirstName").From("User").GroupBy(null).Go();
        });
    }
    ...
    {
        var select = Sql.Select("FirstName").From("User").GroupBy("FirstName").Go();
    ...
    {
        var select = Sql.Select("FirstName")
            .From("User")
    ...
    public void Having_WithNullPredicate_ThrowsArgumentNull()
    {
        Assert.Throws<ArgumentNullException>(() =>
        {
            Sql.Select("FirstName").From("User").GroupBy("FirstName").Having(null).Go();
        });
    }

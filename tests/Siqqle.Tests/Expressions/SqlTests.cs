...
public class SqlTests
{
    ...
    public void Union_WithRealLifeQuery_ReturnsQuery()
    {
        ...
        const string expected =
            "(SELECT [s].[City] FROM [Supplier] [s]) UNION (SELECT [c].[City] FROM [Customer] [c])";
    ...
    public void Select_WithRealLifeQuery_ReturnsQuery()
    {
        ...
        const string expected =
            "SELECT [u].[Id], [u].[Name], [u].[Email], [p].[Avatar], [p].[Birthday] FROM [User] [u] LEFT JOIN [Profile] [p] ON [u].[Id] = [p].[UserId] WHERE [p].[Age] = 18 ORDER BY [u].[Name]";
        Assert.Equal(expected, actual);
    }
    ...
    public void Select_WithGroupByWithRealLifeQuery_ReturnsQuery()
    {
        ...
        const string expected =
            "SELECT [p].[Age], COUNT([p].[Id]) AS [Count] FROM [Profile] [p] GROUP BY ([p].[Age]) HAVING [p].[Age] >= 18";
        Assert.Equal(expected, actual);
    }
    ...
    {
        SqlTable users = "User";
        var actual = Sql.Delete().From(users).Where(SqlExpression.Equal(users + "Id", 5)).ToSql();
    ...
    public void Insert_WithRealLifeQuery_ReturnsQuery()
    {
        ...
        const string expected =
            "INSERT INTO [User] ([User].[Name], [User].[Email]) VALUES ('John Doe', 'john.doe@ac.edu')";
        Assert.Equal(expected, actual);
    }
    ...
    public void Update_WithRealLifeQuery_ReturnsQuery()
    {
        ...
        const string expected =
            "UPDATE [User] SET [User].[Name] = 'John Doe', [User].[Email] = 'john.doe@ac.edu' WHERE [User].[Id] = 5";
        Assert.Equal(expected, actual);
    }
}

...
public class SqlIdentifier : SqlExpression, IEquatable<SqlIdentifier>
{
    ...
    public SqlIdentifier(string identifier)
    {
        ...
        if (identifier.Length == 0)
            throw new ArgumentException(
                "The value of the identifier argument must be a non-empty string.",
                nameof(identifier)
            );
    ...
    /// </summary>
    public override SqlExpressionType ExpressionType => SqlExpressionType.Identifier;
    ...
    public override int GetHashCode()
    {
        unchecked
        {
            return Segments.Aggregate(
                17,
                (current, segment) => current * 23 + segment.GetHashCode()
            );
        }
    }

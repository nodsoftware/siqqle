        ...
        : base(name, precision)
    {
        if (scale != null && precision == null)
            throw new ArgumentException(
                "The scale argument can only be specified if the precision argument is specified.",
                nameof(scale)
            );
        if (scale != null && (scale < 0 || scale > precision))
            throw new ArgumentOutOfRangeException(
                nameof(scale),
                scale,
                "The scale argument must be greater than or equal to zero and less than or equal to the precision argument."
            );
